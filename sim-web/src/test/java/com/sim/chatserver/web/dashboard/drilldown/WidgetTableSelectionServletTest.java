...
public class WidgetTableSelectionServletTest {
    ...
    public void doGet_unauthenticated_returnsUnauthorizedJsonContract() throws Exception {
        ...
        assertEquals("Authentication required.", body.getString("message"));
    }

    @Test
    public void doGet_missingWidgetId_returnsBadRequestJsonContract() throws Exception {
        WidgetTableSelectionServlet underTest = new WidgetTableSelectionServlet();
        ...
        JsonObject body = Json.createReader(new StringReader(out.toString(StandardCharsets.UTF_8))).readObject();
        assertEquals("error", body.getString("status"));
        assertEquals("widgetId required.", body.getString("message"));
    }
